...
class FullLatticeFieldAnalyzer(nn.Module):
    """
    Analyzes the complete, multi-level lattice structure based on the provided diagram.
    This version generates the entire hierarchical lattice and its interconnections.
    """
    def __init__(self, max_seq_len=8192, num_levels=8):
        super().__init__()

        # Generate the full lattice
        self.lattice, self.parent_map = self._generate_full_lattice(max_seq_len, num_levels)

        # The main spine is the first sequence in our lattice
        self.register_buffer('spine', torch.tensor(self.lattice[0], dtype=torch.long))

        # Pre-compute the structure for all nodes in the lattice
        self.lattice_structure = {}
        for level_nodes in self.lattice:
            for pos in level_nodes:
                 if pos < max_seq_len:
    ...
    def get_structure(self, pos: int):
        """Get precomputed structure for a position."""
        return self.lattice_structure.get(pos)

    def _analyze_position(self, pos):
        """Build the ancestor levels for a given position."""
        levels = {0: [pos]}
        visited = {pos}
        current_level_nodes = [pos]
        level = 0

        while current_level_nodes and level < 10:
    ...
    def _get_immediate_ancestors(self, pos):
        """Get pre-computed parents from the parent_map."""
        return self.parent_map.get(pos, [])
        ...
        for spine_pos in relevant_spine:
            ...
            for level in range(structure['max_depth'] + 1):
                ...
                for node in level_nodes:
                    if node < S:
                        # The concept of 'path_counts' is removed in the new model.
                        # We now use a uniform weight for all ancestors.
                        weight = 1.0
...
class PathWeightedLatticeCore(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for spine_pos in relevant_spine:
            ...
            for level in sorted(structure['levels'].keys()):
                if level > 0:
            ...
            # Create a dummy tensor for path weights, as path_counts is no longer used
            path_weights_tensor = torch.ones(len(all_ancestors), device=x.device)
...
class HSTv7_2FullLattice(nn.Module):
    def __init__(
        self,
        vocab_size,
        d_model,
...
if __name__ == '__main__':
    print("=" * 70)
    print("HST-v7.2 FULL LATTICE - Full Model Self-Test")
    print("=" * 70)

    # --- Verify Lattice Structure ---
    print("\n--- Verifying Full Lattice Structure ---")
    analyzer = FullLatticeFieldAnalyzer(max_seq_len=256)
    print(f"Generated Spine: {analyzer.spine.tolist()}")
    pos_to_check = 24
    if pos_to_check in analyzer.lattice_structure:
        structure = analyzer.get_structure(pos_to_check)
        print(f"Ancestors for position {pos_to_check}:")
        for level, nodes in sorted(structure['levels'].items()):
            if level > 0:
                print(f"  Level {level}: {nodes}")
    else:
        print(f"Position {pos_to_check} not found in pre-computed lattice.")


    # --- Test Context Injection ---
    print("\n--- Testing Context Injection Mode ---")
    model_injection = HSTv7_2FullLattice(
        vocab_size=50257,
        d_model=64,
        n_heads=2,
        n_layers=2,
    ...
    model_token = HSTv7_2FullLattice(
        vocab_size=50257,
        d_model=64,
        n_heads=2,
        n_layers=2,
    ...
    print("\n--- Testing Chunk Mode ---")
    # NOTE: Using smaller d_model and n_layers to prevent OOM errors during self-test.
    model_chunk = HSTv7_2FullLattice(
        vocab_size=50257,
        d_model=64,
        n_heads=2,
        n_layers=2,
